...
| Метод | Скорость | Стоимость | Точность | Использование |
|-------|----------|-----------|----------|---------------|
| **БЕЗ LLM** | < 1 ms | $0 (бесплатно) | Оценочная | Анализ, планирование |
| **С LLM** | 1-2 ms (кеш)<br>100-500 ms (без кеша) | ~$0.45/1M токенов | Реальная | Продакшн, качество |

**Результаты БЕЗ LLM** (только токенизация):
- 📊 11,636 токенов → 3,490 токенов (30% сжатие)
- 💰 **Экономия 70%** токенов при 30% коэффициенте
- ⚡ Мгновенная обработка (< 1 мс на файл)
- 🔒 100% локально, без API вызовов

См. подробности в [COMPRESSION_BENCHMARKS.md](COMPRESSION_BENCHMARKS.md) и [USAGE_EXAMPLES.md](USAGE_EXAMPLES.md)
